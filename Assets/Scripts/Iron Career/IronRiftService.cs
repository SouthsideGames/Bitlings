...
/// </summary>
public sealed class IronRiftService
{
    ...
    public IronRiftService(IronCareerRunState state, IronRngStream rng, IronTitleRoller titleRoller)
    {
        _state = state;
        _rng = rng;
    ...
    public IronMonster RollNextWild()
    {
        ...
        {
            Debug.LogWarning($"[IronRiftService] Failed to build wild pool from MonsterCatalog. Error: {ex.Message}");
        }
        ...
        if (all.Count == 0)
        {
            Debug.LogError("[IronRiftService] MonsterLibrary missing or AllAvailable is empty.");
            return null;
        }
