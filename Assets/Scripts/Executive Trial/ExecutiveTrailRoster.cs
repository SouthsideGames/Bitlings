...
public sealed class ExecutiveTrailRoster
{
    private readonly ExecutiveTrialRunState _state;

    private static bool IsForcedEvolutionEligible(ExecutiveTrailMonster monster)
    {
    ...
    public ExecutiveTrailRoster(ExecutiveTrialRunState state)
    {
        _state = state;
    }

    public IReadOnlyList<ExecutiveTrailMonster> Party => _state.party;
    ...
    public void AddMember(ExecutiveTrailMonster m)
    {
    ...
    public void ReplaceMember(int indexToReplace, ExecutiveTrailMonster newMember)
    {
    ...
    public void EnsureHpInitialized(ExecutiveTrailMonster m)
    {
        if (m == null) return;
