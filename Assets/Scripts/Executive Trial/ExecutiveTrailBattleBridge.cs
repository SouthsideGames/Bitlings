...
public sealed class ExecutiveTrailBattleBridge : MonoBehaviour, IBattleRosterProvider, IBattleContext
{
    public interface IExecutiveTrailBattleBridgeHost
    {
    ...
    [Tooltip("If set, this component must implement ExecutiveTrailBattleBridge.IExecutiveTrailBattleBridgeHost.")]
    ...
    private IExecutiveTrailBattleBridgeHost Host => ResolveHost();
    ...
    private IExecutiveTrailBattleBridgeHost ResolveHost()
    {
        if (hostBehaviour is IExecutiveTrailBattleBridgeHost typedHost)
            return typedHost;

        if (hostBehaviour != null)
            Debug.LogWarning($"[ExecutiveTrailBattleBridge] Assigned host '{hostBehaviour.name}' does not implement IExecutiveTrailBattleBridgeHost.");

        var localBehaviours = GetComponents<MonoBehaviour>();
        for (int i = 0; i < localBehaviours.Length; i++)
        {
            if (localBehaviours[i] is IExecutiveTrailBattleBridgeHost localHost)
            {
        ...
        var managerHost = FindFirstObjectByType<ExecutiveTrialManager>(FindObjectsInactive.Include);
        if (managerHost != null)
        {
    ...
    public void BeginIronBattle(BattleManager battle, Action<BattleResult> onEnded)
    {
...
#if UNITY_EDITOR
        DevLog.Log($"[ExecutiveTrailBattleBridge] BeginIronBattle: ironActive={ExecutiveTrialRuntime.IsActive} host={(hostBehaviour ? hostBehaviour.name : "NULL")} useDebugFallback={useDebugFallbackIfNoHost}");
#endif

        if (!ExecutiveTrialRuntime.IsActive)
        {
            Debug.LogError("[ExecutiveTrailBattleBridge] BeginIronBattle called but ExecutiveTrialRuntime is not active. Aborting battle start.");
        ...
        {
            Debug.LogError("[ExecutiveTrailBattleBridge] Party is empty/invalid at battle start. Aborting battle start.");
...
#if UNITY_EDITOR
        DevLog.Log($"[ExecutiveTrailBattleBridge] Pre-Begin snapshot: partyCount={party.Count} wild={(wild != null && wild.def != null ? wild.def.name : "NULL")}");
#endif
        if (wild == null || wild.def == null)
        {
            Debug.LogError("[ExecutiveTrailBattleBridge] Wild combatant is null/invalid. Aborting battle start.");
    ...
    #if UNITY_EDITOR
        DevLog.Log("[ExecutiveTrailBattleBridge] battle.Begin(...) invoked with injected roster provider + context.");
    #endif
...
#if UNITY_EDITOR
        DevLog.Log($"[ExecutiveTrailBattleBridge] carryStatus snapshot = {carryStatus}");    
#endif
    }

    private void NotifyStartFailure(Action<BattleResult> onEnded, string reason, MonsterDataSO wildDef = null, int wildLevel = 1)
    {
        Debug.LogError($"[ExecutiveTrailBattleBridge] Battle start failed: {reason}");
        ...
        {
            Debug.LogError($"[ExecutiveTrailBattleBridge] Start failure callback exception: {ex.Message}");
        }
    ...
    public bool IronWildIsPremium
    {
        ...
        {
            var host = Host;
            if (host is ExecutiveTrialManager mgr) return mgr.IronWildIsPremium;
            return false;
        }
    }
    ...
    {
#if UNITY_EDITOR
        DevLog.Log($"[ExecutiveTrailBattleBridge] OnBattleResolved: victory={outcome.victory} escaped={outcome.escaped} turns={outcome.turnsSurvived}");
#endif
        // Forward to run host (ExecutiveTrialManager) and allow it to update runtime state.
        var host = Host;
        if (host != null)
            host.OnIronBattleResolved(outcome);
    }
    ...
    {
        string g = string.IsNullOrEmpty(ExecutiveTrialRuntime.RunGuid) ? "noguid" : ExecutiveTrialRuntime.RunGuid;
