...
/// <summary>
/// Deterministic wild selection + level scaling for Executive Trial.
/// Produces a single wild snapshot used for BOTH the battle enemy and the hire offer.
/// </summary>
public sealed class ExecutiveTrailRiftService
{
    private readonly ExecutiveTrialRunState _state;
    private readonly ExecutiveTrailRngStream _rng;
    private readonly ExecutiveTrailTitleRoller _titleRoller;
    private string _lastWildMonsterId;

    public ExecutiveTrailRiftService(ExecutiveTrialRunState state, ExecutiveTrailRngStream rng, ExecutiveTrailTitleRoller titleRoller)
    {
        _state = state;
        _rng = rng;
    ...
    public ExecutiveTrailMonster RollNextWild()
    {
        ...
        {
            Debug.LogWarning($"[ExecutiveTrailRiftService] Failed to build wild pool from MonsterCatalog. Error: {ex.Message}");
        }
        ...
        if (all.Count == 0)
        {
            Debug.LogError("[ExecutiveTrailRiftService] MonsterLibrary missing or AllAvailable is empty.");
            return null;
        }
        ...
        // Roll premium: same base chance as normal rifts. Iron species must have premium art.
        bool premium = false;
        ...
        var wild = new ExecutiveTrailMonster(chosen, level, curHp: -1f, locked: title, premium: premium);
    ...
    public static int ComputeWildLevel(int wins, ExecutiveTrailRngStream rng)
    {
