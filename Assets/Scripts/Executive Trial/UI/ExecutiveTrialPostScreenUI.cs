...
/// <summary>
/// Phase 3.A: ExecutiveTrialPostScreenUI
/// Party overview after hire/replace.
/// Shows HP + Title + (single) carried status icon.
/// </summary>
public sealed class ExecutiveTrialPostScreenUI : MonoBehaviour
{
    [Header("Refs")]
    ...
    [SerializeField] private ExecutiveTrialPostCardUI partyCardPrefab;
    ...
    private readonly List<ExecutiveTrialPostCardUI> _spawnedPartyCards = new List<ExecutiveTrialPostCardUI>(3);
    ...
    public void Bind(IReadOnlyList<ExecutiveTrailMonster> party, IronFieldStatusSnapshot carry, int wins)
    {
        // Back-compat path: no outcome provided.
        BindInternal(party, carry, wins, hasOutcome: false, outcome: default);
    }

    public void Bind(IReadOnlyList<ExecutiveTrailMonster> party, IronFieldStatusSnapshot carry, int wins, IronBattleOutcome outcome)
    {
    ...
    private void BindInternal(IReadOnlyList<ExecutiveTrailMonster> party, IronFieldStatusSnapshot carry, int wins, bool hasOutcome, IronBattleOutcome outcome)
    {
        // Header/meta
        ...
        {
            string wildName = (outcome.wildDef != null) ? MonsterNameFormatter.Format(outcome.wildDef, outcome.wildWasPremium) : "Unknown";
        ...
        {
            if (!partyListParent) Debug.LogWarning("[ExecutiveTrialPostScreenUI] partyListParent is not assigned. Party cards will not spawn.");
            if (!partyCardPrefab) Debug.LogWarning("[ExecutiveTrialPostScreenUI] partyCardPrefab is not assigned. Party cards will not spawn.");
        }
#endif
