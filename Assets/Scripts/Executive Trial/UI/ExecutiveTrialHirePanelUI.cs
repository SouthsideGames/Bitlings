...
public sealed class ExecutiveTrialHirePanelUI : MonoBehaviour
{
    [Header("Refs")]
    ...
    public void Bind(ExecutiveTrailMonster offer, bool skipAllowed)
    {
        ClearHireResultVisuals();

        if (portrait) portrait.sprite = offer != null && offer.def ? MonsterNameFormatter.GetIcon(offer.def, offer.isPremium, false) : null;
        if (nameLabel) nameLabel.text = offer != null && offer.def ? MonsterNameFormatter.Format(offer.def, offer.isPremium) : "-";
