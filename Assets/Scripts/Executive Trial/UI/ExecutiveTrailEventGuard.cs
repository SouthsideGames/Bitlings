...
public sealed class ExecutiveTrailEventGuard : MonoBehaviour
{
    ...
    [Header("Forbidden during Executive Trial")]
    ...
    {
        // NOTE: We intentionally subscribe broadly; checks are gated by ExecutiveTrialRuntime.IsActive.
    ...
    private bool IsGuardActive()
    {
        ...
        if (!_executiveTrialManager)
            _executiveTrialManager = FindFirstObjectByType<ExecutiveTrialManager>(FindObjectsInactive.Include);

        return _executiveTrialManager != null && _executiveTrialManager.IsRunActive;
    }
    ...
    {
        // Executive Trial now intentionally banks run rewards (credits/growth cores) on battle resolution.
