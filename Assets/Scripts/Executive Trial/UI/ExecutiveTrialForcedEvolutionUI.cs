...
/// </summary>
public sealed class ExecutiveTrialForcedEvolutionUI : MonoBehaviour
{
    [Header("Refs")]
    ...
    [SerializeField] private ExecutiveTrialEligibleEvolutionItemUI eligiblePrefab;

    [Header("Preview")]
    [SerializeField] private TextMeshProUGUI previewHeaderTMP;
    [SerializeField] private ExecutiveTrialEvolutionPreviewCardUI beforeCard;
    [SerializeField] private ExecutiveTrialEvolutionPreviewCardUI afterCard;
    ...
    private readonly List<ExecutiveTrialEligibleEvolutionItemUI> _spawned = new List<ExecutiveTrialEligibleEvolutionItemUI>(8);
    private readonly List<int> _eligibleIndices = new List<int>(8);

    private int _selectedPartyIndex = -1;

    private bool IsForcedEvolutionEligible(ExecutiveTrailMonster monster)
    {
    ...
    private void Awake()
    {
        if (!manager) manager = FindFirstObjectByType<ExecutiveTrialManager>(FindObjectsInactive.Include);
    ...
    public void Bind(IReadOnlyList<ExecutiveTrailMonster> party)
    {
        ...
        if (warningTMP)
        {
            warningTMP.text = "⚠ Evolution is permanent in Executive Trial. HP carries forward by percent. No healing is granted.";
        }
    ...
    private void RebuildEligibleList(IReadOnlyList<ExecutiveTrailMonster> party)
    {
    ...
    private void RefreshButtonsAndPreview(IReadOnlyList<ExecutiveTrailMonster> party)
    {
        ...
        if (beforeCard) beforeCard.Bind(beforeDef, m.level, m.lockedTitle, m.isPremium);
        if (afterCard) afterCard.Bind(afterDef, m.level, m.lockedTitle, m.isPremium);
        AnimatePreviewSwap();
