...
public class RiftPanelUI : MonoBehaviour
{
    public static RiftPanelUI I { get; private set; }

    [Header("Refs")]
    [SerializeField] private Button riftBtn;
    ...
    [Header("Rift Difficulty Preview")]
    [Tooltip("Optional label: show estimated wild level and expected title count.")]
    [SerializeField] private TextMeshProUGUI riftPreviewLabel;
    [SerializeField] private bool showRiftPreview = true;
    ...
    // ─────────────────────────────────────────────────────────────
    // Rift Bar Buttons + Labels (Icon + TMP under it)
    // ─────────────────────────────────────────────────────────────
    [Header("Rift Bar Buttons")]
    ...
    [SerializeField, Range(0f, 0.5f)] private float postFadeRiftDelay = 0.08f;
    ...
    [SerializeField] private string hardFallbackLine = "What will we rift...";
    ...
    private TextMeshProUGUI riftLabel;
    ...
    void Awake()
    {
        ...
        riftLabel = riftBtn ? riftBtn.GetComponentInChildren<TextMeshProUGUI>() : null;
    ...
    void OnEnable()
    {
        ...
        if (riftBtn)
        {
            riftBtn.onClick.RemoveAllListeners();
            riftBtn.onClick.AddListener(OnClickRift);
        }
        ...
        {
            RiftManager.I.OnStateChanged += OnRiftStateChanged;
            GameEvents.EnergyChanged += RefreshEnergy;
        }

        RiftManager.OnEnergyGained += OnEnergyGained;
        ...
        GameEvents.OnRiftAutoModeChanged += ApplyCloseLock;
        ...
        ApplyRiftButtonBattleVisibility(IsInBattle());
        ...
        RefreshAll();
        RefreshRiftBoostIconsAndTooltips(force: true);
    ...
    void OnDisable()
    {
        ...
        if (riftBtn) riftBtn.onClick.RemoveAllListeners();
    ...
    void Update()
    {
        ...
        if (_etaTickAccum >= 1f)
        {
            _etaTickAccum = 0f;
            UpdateEnergyEtaUI();
            RefreshRiftBoostIconsAndTooltips(force: false);
        }
    }
    ...
    void OnResourcesChanged()
    {
        ...
        RefreshRiftButtonInteractivity();

        RefreshRiftDifficultyPreview();
    }
    ...
    // ─────────────────────────────────────────────────────────────
    // Rift Difficulty Preview (NO FATIGUE)
    // ─────────────────────────────────────────────────────────────
    void RefreshRiftDifficultyPreview()
    {
        RefreshBlinderDifficultyInfo();

        if (!riftPreviewLabel) return;
        if (!showRiftPreview)
        {
            riftPreviewLabel.gameObject.SetActive(false);
            return;
        }

        // Only meaningful when we're about to start an rift.
        ...
        if (RiftManager.I != null)
        {
            ...
            boss = (d != null) && d.riftsSinceBoss >= cadence;
            bossBonus = RiftManager.I.BossLevelBonusPreview;
        }
        ...
        int knownTitles = (RiftManager.I != null && RiftManager.I.WildActiveTitles != null)
            ? RiftManager.I.WildActiveTitles.Count
            : -1;
        ...
        riftPreviewLabel.text = $"Wild Lv ~{est}{bossTag}  •  Titles: {titleText}";
        riftPreviewLabel.gameObject.SetActive(true);
    }
    ...
    // ─────────────────────────────────────────────────────────────
    // Rift Boost Icons + Tooltips
    // ─────────────────────────────────────────────────────────────
    void RefreshRiftBoostIconsAndTooltips(bool force)
    {
        UpdateEnergyTooltip();

        long flyerRem = (RiftManager.I != null) ? RiftManager.I.GetFlyerSecondsRemaining() : -1;
        ...
        if (flyerActive)
        {
            var cur = RiftManager.I != null ? RiftManager.I.CurrentFlyer : null;
    ...
    void UpdateFlyerTooltip(object flyerObj, long remainingSeconds)
    {
        ...
        if (RiftManager.I != null && RiftManager.I.CurrentFlyer != null)
            typeName = RiftManager.I.CurrentFlyer.type.ToString();
    ...
    string BuildEnergyEtaStringFallback(int cur, int max)
    {
        ...
        int seconds = RiftManager.I != null
            ? RiftManager.I.GetSecondsUntilFull()
    ...
    {
        BattleLogger.Log($"Win Streak: {value}", LogScope.Rift);
    }
    ...
    void LogCurrentWinStreak(string prefix = null)
    {
        ...
        BattleLogger.Log($"{label}: {streak}", LogScope.Rift);
    }
...
if (blinderBackground)
{
    ...
    void RefreshButtonAndLabel()
    {
        ...
        if (!riftLabel)
            riftLabel = riftBtn.GetComponentInChildren<TextMeshProUGUI>();

        if (riftLabel)
        {
            if (auto)
                riftLabel.text = "AUTO: ON";
            else if (nextFree)
                riftLabel.text = "NEXT";
            else
                riftLabel.text = "RIFT";
    ...
    void RefreshRiftButtonInteractivity()
    {
        if (!riftBtn) return;
        ...
        //  - enough energy to pay the cost, unless the next rift is free
    ...
    {
        if (RiftManager.I != null) return false;

        int current = Mathf.Max(0, ResourceBank.Get(ResourceType.Energy));
        int needed = Mathf.Max(1, GetRiftCost());
    ...
    void OnRiftStateChanged()
    {
        bool inBattle = IsInBattle();
        ApplyRiftButtonBattleVisibility(inBattle);
    ...
    void OnClickRift()
    {
        ...
        AudioManager.I?.PickRiftBattleMusicOnButtonPress(playImmediately: true);
        ...
        {
            if (_fadeCo != null) StopCoroutine(_fadeCo);
            _fadeCo = StartCoroutine(Co_FadeOutBlinderThenStartRift());
        }
        else
        {
            RequestRiftTap();
    ...
    IEnumerator Co_FadeOutBlinderThenStartRift()
    {
        _isFading = true;
        RefreshButtonAndLabel();
        ...
        if (postFadeRiftDelay > 0f)
            yield return new WaitForSecondsRealtime(postFadeRiftDelay);
...
if (target > 0.01f)
{
    ...
    {
        bool premium = RiftManager.I != null && RiftManager.I.CurrentWildIsPremium;
    ...
    public void ShowHireDecision(MonsterDataSO def, int level, bool isPremium)
    {
        ...
        {
            RiftManager.I?.OnHireDecisionResolved(false, false);
            return;
        }
    ...
    void OnClickHireYes()
    {
        ...
        bool success = false;
        if (RiftManager.I != null && _pendingHireDef != null)
            success = RiftManager.I.TryCaptureFromDecision(_pendingHireDef, _pendingHireLevel);
    ...
    void OnClickHireContinue()
    {
        ...
        RiftManager.I?.OnHireDecisionResolved(_hireChoseYes, _hireCaptureSucceeded);
    ...
    void RefreshBlinderTint()
    {
        ...
        {
            var lure = (RiftManager.I != null) ? RiftManager.I.CurrentFlyer : null;
            if (lure != null)
    ...
    bool IsBossImminent()
    {
        ...
        int since = Mathf.Max(0, data.riftsSinceBoss);
    ...
    // ─────────────────────────────────────────────────────────────
    // RiftManager passthrough helpers
    // ─────────────────────────────────────────────────────────────
    bool IsInBattle() => RiftManager.I != null && RiftManager.I.IsInBattle;
    bool IsAutoMode() => RiftManager.I != null && RiftManager.I.IsAutoMode;
    bool NextRiftIsFree() => RiftManager.I != null && RiftManager.I.NextRiftIsFree;
    bool HasEnergy() => RiftManager.I != null && RiftManager.I.HasEnergy();
    public int GetEnergyPoints() => RiftManager.I != null ? RiftManager.I.GetEnergyPoints() : 0;
    public int GetRiftMax() => RiftManager.I != null ? RiftManager.I.GetRiftMax() : 0;
    public int GetRiftCost() => RiftManager.I != null ? RiftManager.I.GetRiftCost() : 0;

    void RequestRiftTap() => RiftManager.I?.RequestRiftTap();
    public void OnClickToggleAuto() => RiftManager.I?.ToggleAutoMode();
    ...
    void PlayEnergyGainedFX()
    {
        ...
        var rt = riftBtn.transform as RectTransform;
        if (!rt) return;
    ...
    private void ApplyRiftButtonBattleVisibility(bool inBattle)
    {
        if (!riftBtn) return;
        var btnGO = riftBtn.gameObject;
        if (!btnGO) return;
