...
public class TypeResistBooster : MonoBehaviour
{
    ...
    private void RefreshInteractability(bool hard)
    {
        ...
        bool haveItem = !consumeItem || ResourceBank.Get(ResourceType.Sigils) > 0;
    ...
    private void OnPress()
    {
        ...
        if (consumeItem)
        {
            spent = ResourceBank.TrySpend(ResourceType.Sigils, 1);
            if (!spent)
            {
                RefreshCounts();
                RefreshInteractability(true);
        ...
        {
            ResourceBank.Add(ResourceType.Sigils, 1);
