...
public static class BossEncounterHelper
{
    // ----- Public API -----

    /// Decide if the next encounter should be a boss.
    public static bool ShouldSpawnBoss(int encountersSinceBoss, int bossEveryN)
    {
    ...
    /// Returns the new encountersSinceBoss and (optionally) updates lastBossId when a boss spawned.
    public static void AfterBattle(ref int encountersSinceBoss, bool wasBoss, MonsterDataSO bossUsed, ref string lastBossId)
    {
        if (wasBoss)
        {
